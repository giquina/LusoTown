  ...
  async fixBuildScripts() {
    await this.runFix('Build scripts validation', () => {
      ...
      const requiredScripts = {
        ...
        'deploy:auto': 'npm run auto-fix && npm run build && npm run deploy'
      };
...
const inter = shouldLoadFonts ? Inter({`
            ...
            if (layoutContent.includes('className={`${inter.variable} ${poppins.variable}`}')) {
              layoutContent = layoutContent.replace(
            ...
            if (layoutContent.includes('className={`${inter.className} antialiased`}')) {
              layoutContent = layoutContent.replace(
    ...
    await this.runFix('Framer Motion SSR compatibility', () => {
      ...
      if (fs.existsSync(nextConfigPath)) {
        ...
        if (!nextConfig.includes('esmExternals')) {
          // Find the experimental section and add esmExternals
