...
export const getTransportServices = async (): Promise<TransportService[]> => {
  try {
...
export const getTransportVehicles = async (): Promise<TransportVehicle[]> => {
  try {
    const { data, error } = await supabase
...
export const getTransportDrivers = async (languages?: string[]): Promise<TransportDriver[]> => {
  try {
    let query = supabase
...
export const getPricingTiers = async (): Promise<TransportPricingTier[]> => {
  try {
    const { data, error } = await supabase
      .from('transport_pricing_tiers')
      .select('*')
      .eq('is_active', true)
...
export const getPeakTimes = async (): Promise<TransportPeakTime[]> => {
  try {
    const { data, error } = await supabase
      .from('transport_peak_times')
      .select('*')
      .eq('is_active', true)
...
export const createTransportBooking = async (
  bookingData: Omit<TransportBooking, 'id' | 'booking_reference' | 'created_at' | 'updated_at'>
): Promise<{ success: boolean; booking?: TransportBooking; error?: string }> => {
  try {
    const { data, error } = await supabase
...
// Get user bookings
export const getUserTransportBookings = async (userId: string): Promise<TransportBooking[]> => {
  try {
    const { data, error } = await supabase
      .from('transport_bookings')
      .select(`
        *,
        transport_services!inner(service_name, service_type),
        transport_vehicles(make, model, category),
        transport_drivers(first_name, last_name)
      `)
...
export const getAvailableVehicles = async (
  ...
  try {
    ...
    const availableVehicles: TransportVehicle[] = []
...
export const getAvailableDrivers = async (
  ...
  try {
    const drivers = await getTransportDrivers(languages)
    const availableDrivers: TransportDriver[] = []
