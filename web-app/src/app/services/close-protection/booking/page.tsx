...
export default function CloseProtectionBookingPage() {
  ...
  const [selectedPackage, setSelectedPackage] = useState(packageParam || 'discrete-protection');
  ...
  const selectedPackageData = protectionPackages.find(pkg => pkg.id === selectedPackage);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  placeholder={isPortuguese ? 'Londres, Reino Unido' : 'London, UK'}
                  className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-premium-500 focus:border-premium-500"
                  required
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  {isPortuguese ? 'Número de Pessoas a Proteger' : 'Number of People to Protect'} *
                </label>
                <select
      ...
      case 3:
        return (
          <div className="space-y-6">
            ...
            <div>
              ...
              <textarea
                ...
                placeholder={isPortuguese 
                  ? 'Qualquer requisito específico, restrições ou considerações especiais...'
                  : 'Any specific requirements, restrictions, or special considerations...'}
                className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-premium-500 focus:border-premium-500"
              />
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                {isPortuguese ? 'Preferências Culturais/Linguísticas' : 'Cultural/Language Preferences'}
              </label>
              <textarea
                ...
                rows={3}
                placeholder={isPortuguese 
                  ? 'Preferências de idioma, sensibilidades culturais, protocolos empresariais...'
                  : 'Language preferences, cultural sensitivities, business protocols...'}
                className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-premium-500 focus:border-premium-500"
              />
            </div>
          </div>
                ...
                <div key={step} className="flex items-center">
                  ...
                  {step < 3 && (
                    <div className={`w-24 h-1 mx-4 ${
                      currentStep > step ? 'bg-premium-600' : 'bg-gray-200'
                    }`} />
                  )}
                </div>
              ))}
            ...
            <div className="flex justify-between">
              ...
              {currentStep < 3 ? (
                <button
                  ...
                  className="px-6 py-3 bg-premium-600 text-white rounded-xl font-medium hover:bg-premium-700"
                >
                ...
                <button
                  ...
                  className="px-8 py-3 bg-premium-600 text-white rounded-xl font-medium hover:bg-premium-700 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isSubmitting
                    ? (isPortuguese ? 'Enviando...' : 'Submitting...')
                    : (isPortuguese ? 'Solicitar Consulta' : 'Request Consultation')
                  }
                </button>
              )}
            </div>
