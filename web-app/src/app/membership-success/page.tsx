...
const MEMBERSHIP_CONFIG = {
  ...
  social: {
    ...
    icon: HeartIcon,
    color: 'rose',
    description: 'Your application to join our elite Lusophone dating community',
    benefits: [
  ...
  community: {
    ...
    icon: UserGroupIcon,
    color: 'blue',
    description: 'Your application to join the UK\'s premier Portuguese-speaking community',
    benefits: [
  ...
  business: {
    ...
    color: 'emerald',
    description: 'Your application to join the UK\'s premier Lusophone business network',
    benefits: [
...
function MembershipSuccessPageInner() {
  ...
  const membershipType = searchParams.get('type') as keyof typeof MEMBERSHIP_CONFIG || 'community'
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem(`${membershipType}-membership-application`)
      if (saved) {
      ...
      <div className="max-w-4xl mx-auto">
              ...
              {applicationData && (
                <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-medium">{applicationData.currentCity}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-600">Submitted:</span>
                    <span className="font-medium">{new Date().toLocaleDateString()}</span>
                  </div>
                  {applicationData.heritage && applicationData.heritage.length > 0 && (
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600">Heritage:</span>
                      <span className="font-medium">{applicationData.heritage.length} selected</span>
                    </div>
                  )}
                </div>
              )}
        ...
        <Card className="mb-8">
          ...
          <CardContent>
            <div className="text-center mb-6">
              <p className="text-gray-600 mb-4">
                While you wait for approval, explore what makes our community special:
              </p>
              
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="text-center">
                  <div className="text-3xl mb-2">🌍</div>
                  <h4 className="font-semibold text-gray-900 mb-1">Global Lusophone Network</h4>
                  <p className="text-sm text-gray-600">
                    Connect with Portuguese speakers from ALL backgrounds across the UK
                  </p>
                </div>
                
                <div className="text-center">
                  <div className="text-3xl mb-2">🎭</div>
                  <h4 className="font-semibold text-gray-900 mb-1">Cultural Celebrations</h4>
                  <p className="text-sm text-gray-600">
                    From Fado to Samba, Kizomba to Morna - all Portuguese cultures celebrated
                  </p>
                </div>
                
                <div className="text-center">
                  <div className="text-3xl mb-2">🤝</div>
                  <h4 className="font-semibold text-gray-900 mb-1">Mutual Support</h4>
                  <p className="text-sm text-gray-600">
                    A community that helps each other thrive in the United Kingdom
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="text-center">
          <div className="space-y-4">
            ...
            <p className="text-sm text-gray-500">
              Automatically redirecting to dashboard in {timeUntilRedirect} seconds...
            </p>
          </div>
        </div>
        ...
        <div className="text-center mt-12 pt-8 border-t border-gray-200">
          ...
          <p className="text-gray-600 max-w-2xl mx-auto">
            Your application represents another step in strengthening the Portuguese-speaking community 
            across the United Kingdom. We look forward to welcoming you and celebrating our shared heritage together.
          </p>
        </div>
      </div>
    </div>
