...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalFreeDays =
      referrals?.reduce((total, referral) => {
        const freeMonthRewards = referral.referral_rewards.filter(
          (reward: any) =>
            reward.reward_type === "free_month" && reward.is_redeemed
        );
        return (
          total +
          freeMonthRewards.reduce(
            (sum: number, reward: any) => sum + reward.reward_value,
            0
          )
        );
      }, 0) || 0;
    ...
    const actualPosition =
      leaderboardPosition >= 0 ? leaderboardPosition + 1 : null;
    ...
    return NextResponse.json({
      ...
      activeReferrals,
      pendingReferrals,
      totalEarnings: totalFreeDays * 25, // £25 value per free month
      totalFreeDays,
