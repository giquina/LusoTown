...
function SearchContent() {
  ...
  const getFilteredResults = () => {
    ...
    return searchResults.all.filter(result => categoryData.type.includes(result.type))
  }
                          ...
                          {categories.map(category => (
                            ...
                            >
                              ...
                              {category.id === 'all' && totalResults > 0 && (
                                <span className="ml-2 text-xs opacity-75">({totalResults})</span>
                              )}
                              {category.id === 'events' && searchResults.events.length > 0 && (
                                <span className="ml-2 text-xs opacity-75">({searchResults.events.length})</span>
                              )}
                              {category.id === 'businesses' && searchResults.businesses.length > 0 && (
                                <span className="ml-2 text-xs opacity-75">({searchResults.businesses.length})</span>
                              )}
                              {category.id === 'groups' && searchResults.groups.length > 0 && (
                                <span className="ml-2 text-xs opacity-75">({searchResults.groups.length})</span>
                              )}
                              {category.id === 'pages' && searchResults.pages.length > 0 && (
                                <span className="ml-2 text-xs opacity-75">({searchResults.pages.length})</span>
                              )}
                            </button>
                          ))}
                        </div>
                  ...
                  {isLoading ? (
                                ...
                                <div className="flex flex-wrap gap-2 mt-3">
                                  ...
                                  {result.tags.length > 4 && (
                                    <span className="text-xs text-gray-500">
                                      +{result.tags.length - 4} {language === 'pt' ? 'mais' : 'more'}
                                    </span>
                                  )}
                                </div>
                              )}
                    ...
                    <div className="text-center py-20">
                      ...
                      <div className="space-y-2 text-sm text-gray-500">
                        <p>{language === 'pt' ? 'Sugestões:' : 'Suggestions:'}</p>
                        <ul className="space-y-1">
                          <li>• {language === 'pt' ? 'Verifique a ortografia' : 'Check your spelling'}</li>
                          <li>• {language === 'pt' ? 'Use termos mais gerais' : 'Use more general terms'}</li>
                          <li>• {language === 'pt' ? 'Tente palavras-chave diferentes' : 'Try different keywords'}</li>
                        </ul>
                      </div>
                    </div>
                  ) : (
