...
class ReferralService {
  ...
  const baseUrl = typeof window !== 'undefined' ? window.location.origin : (process.env.NEXT_PUBLIC_SITE_URL || require('@/config/site').SITE_URL)
    ...
    const messages = {
      en: {
        whatsapp: `Join me on LusoTown - the Portuguese community platform in the U.K.! Use my code ${code} and get 25% off your first month. Perfect for connecting with Portuguese speakers. ${referralUrl}`,
        email: {
          subject: 'Join the Portuguese Community in the U.K. - LusoTown',
          body: `Hi!\n\nI wanted to invite you to join LusoTown, the Portuguese community platform I've been using in the U.K.. It's been amazing for connecting with other Portuguese speakers, finding cultural events, and accessing Portuguese-speaking services.\n\nUse my referral code ${code} when you sign up and you'll get 25% off your first month!\n\nJoin here: ${referralUrl}\n\nLooking forward to seeing you in the community!\n\nBest regards`
        },
