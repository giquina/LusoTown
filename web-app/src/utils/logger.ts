...
class LusoTownLogger {
  ...
  constructor() {
    ...
    this.isDebugEnabled =
      this.isDevelopment ||
      Boolean(typeof window !== "undefined" && (window as any).LUSOTOWN_DEBUG);
  }
  ...
  heritageRespectViolation(
    violation: string,
    details: string,
    meta: LogMeta = {}
  ) {
    this.warn(
      `Heritage Respect Protocol Violation: ${violation} - ${details}`,
      {
        ...meta,
        area: "cultural",
  ...
  lusophoneCommunityEngagement(
    community: CulturalContext,
    engagement: string,
    meta: LogMeta = {}
  ) {
