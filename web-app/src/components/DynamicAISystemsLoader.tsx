  ...
  return (
    <div className="flex flex-col items-center justify-center p-8 bg-gradient-to-br from-primary-50 to-secondary-50 rounded-lg border border-primary-200">
      ...
      <div className="mt-4 flex items-center gap-2 text-xs text-primary-600">
        ...
        <span>
          {language === "pt"
            ? "Processamento Cultural Avançado"
            : "Advanced Cultural Processing"}
        </span>
      </div>
    </div>
  );
...
export default function DynamicAISystemsLoader({
  ...
  const { language } = useLanguage();
  const [loadedSystems, setLoadedSystems] = useState<Set<AISystemType>>(
    new Set()
  );
          ...
          >
            ...
            <div className="p-4">
              {error ? (
                ...
                <Suspense
                  fallback={
                    <AISystemLoader type={systemType} title={config.title} />
                  }
                >
                  <ErrorBoundary
      ...
      <div className="mt-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 text-sm text-gray-600">
            <span>
              {language === "pt" ? "Carregados:" : "Loaded:"}{" "}
              {loadedSystems.size}/{systems.length}
            </span>
            <span>
