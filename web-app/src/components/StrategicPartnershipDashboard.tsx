            ...
            <div className="space-y-4">
              {outreachPipeline.filter(item => item.priority === 'high').slice(0, 3).map((item) => (
                <div key={item.id} className="flex items-start space-x-4 p-4 bg-red-50 rounded-lg border border-red-100">
                  <ExclamationTriangleIcon className="w-5 h-5 text-coral-600 mt-1" />
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-secondary-600 mt-1">{item.institutionType}</p>
                    <p className="text-sm text-action-600 font-medium mt-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {recommendations.slice(0, 4).map((rec, index) => (
                <div key={index} className="p-4 border border-secondary-200 rounded-lg hover:shadow-md transition-shadow">
                  ...
                  <p className="text-sm text-secondary-600 mb-3">{rec.rationale}</p>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-action-600 font-medium">{formatCurrency(rec.expectedValue)}</span>
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-secondary-600">{strategy.targetInstitutions.length} {language === 'pt' ? 'instituições alvo' : 'target institutions'}</p>
                      </div>
                    </div>
                    ...
                    <ul className="text-xs text-secondary-600 space-y-1">
                      {strategy.strategicObjectives.slice(0, 3).map((objective, index) => (
                        <li key={index} className="flex items-center">
                          <CheckCircleIcon className="w-3 h-3 text-action-500 mr-2 flex-shrink-0" />
                          {objective}
                        </li>
                      ))}
                    </ul>
                ...
                <div key={item.id} className="p-6 hover:bg-secondary-50 transition-colors">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-secondary-600 mb-3">{item.institutionType}</p>
                      
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        <div>
                          <p className="text-xs text-gray-500">{language === 'pt' ? 'Valor Proposto' : 'Proposed Value'}</p>
                          <p className="font-medium text-action-600">{formatCurrency(item.proposalValue)}</p>
                        </div>
                        <div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-secondary-100 p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-action-600">{language === 'pt' ? 'mensal' : 'monthly'}</p>
                  </div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-secondary-100 p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-action-600">/5 {language === 'pt' ? 'estrelas' : 'stars'}</p>
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                <div className="flex items-center mb-2">
                  <SparklesIcon className="w-5 h-5 text-action-600 mr-2" />
                ...
                <div className="text-xs text-action-600">
