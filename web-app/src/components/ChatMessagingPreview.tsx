...
export default function ChatMessagingPreview() {
  ...
  const [activeTab, setActiveTab] = useState<'live' | 'groups' | 'events'>('live')
  ...
  const mockMessages: ChatMessage[] = useMemo(() => [
    ...
    },
    {
      ...
      type: 'cultural',
      emotes: [':saudade:', ':cafe:'],
      reactions: [{ emoji: '☕', count: 6 }, { emoji: '🤗', count: 11 }]
    },
    {
      id: '5',
      user: {
          ...
          <div className="flex items-center justify-center mt-6 space-x-6">
            ...
            <div className="flex items-center">
              ...
              <span className="text-sm text-gray-600">
                {viewerCount.toLocaleString()} {language === 'pt' ? 'online' : 'online'}
              </span>
            </div>
            <div className="flex items-center">
            ...
            <div className="border-b border-gray-200 bg-gray-50">
              <div className="flex space-x-8 px-6 py-4">
                {(['live', 'groups', 'events'] as const).map((tab) => (
                  ...
                  >
                    {tab === 'live' && <ChatBubbleLeftRightIcon className="w-4 h-4" />}
                    {tab === 'groups' && <UserGroupIcon className="w-4 h-4" />}
                    {tab === 'events' && <CalendarDaysIcon className="w-4 h-4" />}
                    <span>
                      {tab === 'live' && (language === 'pt' ? 'Chat Ao Vivo' : 'Live Chat')}
                      {tab === 'groups' && (language === 'pt' ? 'Grupos' : 'Groups')}
                      {tab === 'events' && (language === 'pt' ? 'Eventos' : 'Events')}
                    </span>
                  </button>
                ))}
              </div>
              ...
              <div className="lg:col-span-2 flex flex-col">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-4">
                      <ShieldCheckIcon 
                        ...
                        onClick={() => setShowModerationFeatures(!showModerationFeatures)}
                      />
                      <span className="text-white text-sm">
                        {mockMessages.length} {language === 'pt' ? 'mensagens' : 'messages'}
                      </span>
                    </div>
                  </div>
                </div>
                    ...
                    {mockMessages.slice(0, 4).map((message, index) => (
                      <motion.div
                        ...
                        className={`flex space-x-3 ${
                          index === currentMessageIndex ? 'ring-2 ring-primary-200 rounded-lg p-2' : ''
                        }`}
                      >
                        {/* Avatar */}
                        <div className="flex-shrink-0">
                          <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white text-sm font-bold ${getRegionColor(message.user.region)}`}>
                            {message.user.avatar}
                          </div>
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="text-xs text-gray-400">
                              {new Intl.RelativeTimeFormat(language, { numeric: 'auto' }).format(
                                Math.floor((message.timestamp.getTime() - Date.now()) / 60000),
                                'minute'
                              )}
                            </span>
                          </div>
                ...
                <div className="px-6 py-4 border-t border-gray-200 bg-white">
                  ...
                  <div className="flex items-center space-x-2 mt-3">
                    ...
                    {[':saudade:', ':festa:', ':futebol:', ':fado:', ':cafe:'].map((emote, idx) => (
                      <button
                        key={idx}
                          ...
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-gray-500">
                                {group.memberCount} {language === 'pt' ? 'membros' : 'members'}
                              </p>
                            </div>
                          </div>
                          {group.isActive && (
                    ...
                    <div className="space-y-2">
                      {mockMessages.slice(0, 6).map((message) => (
                        <div key={message.id} className="flex items-center space-x-2">
                          <div className={`w-6 h-6 rounded-full flex items-center justify-center text-white text-xs ${getRegionColor(message.user.region)}`}>
                            {message.user.avatar}
                          </div>
                      ...
                      >
                        ...
                        <div className="space-y-2 text-sm">
                          <div className="flex items-center text-secondary-700">
                            ...
                            {language === 'pt' ? 'IA Anti-Toxicidade' : 'AI Anti-Toxicity'}
                          </div>
                          <div className="flex items-center text-secondary-700">
                            <CheckCircleIcon className="w-4 h-4 mr-2" />
                            {language === 'pt' ? 'Filtros Culturais' : 'Cultural Filters'}
                          </div>
                          <div className="flex items-center text-secondary-700">
                            <CheckCircleIcon className="w-4 h-4 mr-2" />
                            {language === 'pt' ? 'Verificação Manual' : 'Manual Review'}
                          </div>
                        </div>
                      </motion.div>
                    )}
                    ...
                    <div className="space-y-3">
                      <div className="p-3 bg-white rounded-lg border">
                        <p className="font-medium text-sm text-gray-900">
                          {language === 'pt' ? 'Portugal vs França' : 'Portugal vs France'}
                        </p>
                        <p className="text-xs text-gray-500 mt-1">
                          23 {language === 'pt' ? 'participantes' : 'participants'}
                        </p>
                      </div>
                      <div className="p-3 bg-white rounded-lg border">
                        <p className="font-medium text-sm text-gray-900">
                          {language === 'pt' ? 'Fado Night Soho' : 'Fado Night Soho'}
                        </p>
                        <p className="text-xs text-gray-500 mt-1">
                          18 {language === 'pt' ? 'participantes' : 'participants'}
                        </p>
                      </div>
                      <div className="p-3 bg-white rounded-lg border">
                        <p className="font-medium text-sm text-gray-900">
                          {language === 'pt' ? 'Business Networking' : 'Business Networking'}
                        </p>
                        <p className="text-xs text-gray-500 mt-1">
                          31 {language === 'pt' ? 'participantes' : 'participants'}
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          <div className="bg-gradient-to-r from-primary-500 to-secondary-500 rounded-2xl p-8 text-white">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8 pt-8 border-t border-white/20">
              ...
              <div className="text-center">
                ...
                <p className="text-sm opacity-90">
                  {language === 'pt' ? 'Eventos Integrados' : 'Event Integration'}
                </p>
              </div>
            </div>
          </div>
