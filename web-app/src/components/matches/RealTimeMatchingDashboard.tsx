...
import {
  ...
  type MatchingResult,
  type RealTimeMatchingMetrics,
} from "@/services/AdvancedMatchingAlgorithms";
...
export default function RealTimeMatchingDashboard({
  ...
  const getUpdateInterval = (
    speed: typeof matchingState.matchingSpeed
  ): number => {
    switch (speed) {
      case "slow":
  ...
  const findMatches = useCallback(async () => {
    ...
    filters.culturalDepthRange,
    filters.focusAreas,
    filters.includeNewUsers,
  ]);
  ...
  const getStatusIcon = () => {
    switch (matchingState.optimizationStatus) {
      case "running":
        return <BoltSolid className="w-5 h-5 text-yellow-500 animate-pulse" />;
      case "complete":
        return <SparklesSolid className="w-5 h-5 text-green-500" />;
      case "error":
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-purple-50 to-blue-50 rounded-2xl p-6 border border-purple-100">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900">
                {language === "pt"
                  ? "Matches Inteligentes em Tempo Real"
                  : "Real-Time Intelligent Matching"}
              </h2>
              <p className="text-sm text-gray-600">
                {language === "pt"
                  ? `${
                      matchingState.currentMatches.length
                    } matches encontrados • Atualizado ${formatTimeSince(
                      matchingState.lastUpdate
                    )}`
                  : `${
                      matchingState.currentMatches.length
                    } matches found • Updated ${formatTimeSince(
                      matchingState.lastUpdate
                    )}`}
              </p>
            </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="ml-auto flex items-center gap-2 text-sm text-gray-600">
            ...
            {language === "pt" ? "Atualiza a cada" : "Updates every"}{" "}
            {getUpdateInterval(matchingState.matchingSpeed) / 1000}s
          </div>
        </div>
      </div>
          ...
          >
            ...
            <div className="mt-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                {Object.entries(
                  matchingState.metrics.culturalDepthDistribution
                ).map(([depth, count]) => (
                  <div
                    key={depth}
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      handleFilterChange({
                        culturalDepthRange: [
                          newMin,
                          Math.max(newMin, filters.culturalDepthRange[1]),
                        ],
                      });
                    }}
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      handleFilterChange({
                        culturalDepthRange: [
                          Math.min(filters.culturalDepthRange[0], newMax),
                          newMax,
                        ],
                      });
                    }}
            ...
            <div className="mt-6 flex flex-wrap gap-4">
              <label className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) =>
                    handleFilterChange({ includeNewUsers: e.target.checked })
                  }
                  className="rounded border-gray-300 text-purple-600 focus:ring-purple-500"
                />
                <span className="text-sm text-gray-700">
                  {language === "pt"
                    ? "Incluir novos utilizadores"
                    : "Include new users"}
                </span>
              </label>
              ...
              <label className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) =>
                    handleFilterChange({ realTimeUpdates: e.target.checked })
                  }
                  className="rounded border-gray-300 text-purple-600 focus:ring-purple-500"
                />
                <span className="text-sm text-gray-700">
      ...
      <div className="space-y-4">
        ...
        {/* Empty State */}
        {matchingState.currentMatches.length === 0 &&
          !matchingState.isActive && (
