...
export default function SaudadeCompatibilityEngine({
  ...
  const [compatibleMatches, setCompatibleMatches] = useState<CompatibilityMatch[]>([]);
  ...
  const calculateSaudadeCompatibility = (
    ...
    const intensityDiff = Math.abs(userSaudade.saudadeIntensity - potentialSaudade.saudadeIntensity);
    const saudadeAlignment = Math.max(0, 100 - (intensityDiff * 10));

    // Cultural Depth Compatibility
    const culturalDepthDiff = Math.abs(user.overallCulturalDepth - potential.overallCulturalDepth);
    ...
    return [
      ...
      },
      {
        ...
        saudadeProfile: {
          ...
          copingMechanisms: ['portuguese_community', 'cook_portuguese', 'visit_portugal'],
      ...
      },
      {
        ...
        saudadeProfile: {
          ...
          frequency: 'constant',
          triggers: ['fado_music', 'grandmother_recipes', 'ocean_waves', 'church_bells'],
          copingMechanisms: ['listen_fado', 'cook_portuguese', 'call_family', 'embrace_sadness'],
          ...
          emotionalCompatibilityType: 'Alma Saudosa',
          supportNeeds: ['understanding_saudade', 'cultural_healing', 'share_traditions'],
          culturalHealingActivities: ['Fado therapy', 'Traditional cooking', 'Island festivals'],
        },
        culturalProfile: {
          ...userProfile,
  ...
  const generateSupportStrengths = (
    ...
    if (user.culturalSupport === 'high' && potential.culturalSupport === 'high') {
      strengths.push(
        lang === 'pt'
    ...
    if (user.languageEmotionalAttachment >= 8 && potential.languageEmotionalAttachment >= 8) {
      strengths.push(
        lang === 'pt'
  ...
  const generatePotentialChallenges = (
    ...
    const integrationDiff = Math.abs(user.integrationBalance - potential.integrationBalance);
    ...
    if (user.culturalSupport === 'independent' && potential.culturalSupport === 'high') {
      challenges.push(
        lang === 'pt'
            ...
            >
              ...
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-1">
                  {match.saudadeProfile.triggers.slice(0, 3).map((trigger, idx) => (
                    <span
                      key={idx}
                      className="text-xs bg-primary-100 text-primary-700 px-2 py-1 rounded"
                    >
                      {trigger === 'fado_music' ? (language === 'pt' ? 'Fado' : 'Fado') :
                       trigger === 'family_voices' ? (language === 'pt' ? 'Família' : 'Family') :
                       trigger === 'cook_portuguese' ? (language === 'pt' ? 'Culinária' : 'Cooking') :
                       trigger}
                    </span>
                  ))}
                </div>
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-bold text-primary-900">
                {language === 'pt' ? 'Análise de Compatibilidade Detalhada' : 'Detailed Compatibility Analysis'}
              </h3>
              <button
            ...
            {selectedMatch.compatibilityResult.potentialChallenges.length > 0 && (
              <div className="mb-6">
                <h4 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
                  <ChatBubbleLeftRightIcon className="w-5 h-5 text-orange-500" />
                  {language === 'pt' ? 'Áreas para Conversação' : 'Areas for Discussion'}
                </h4>
                <div className="space-y-2">
            ...
            >
              {language === 'pt' ? 'Iniciar Conversa com Base na Análise' : 'Start Conversation Based on Analysis'}
            </button>
          </motion.div>
        </motion.div>
      )}
