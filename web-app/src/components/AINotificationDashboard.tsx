...
} from "@/services/AINotificationEngine";
import {
...
export default function SmartNotificationDashboard() {
  ...
  const [predictions, setPredictions] = useState<
    Record<string, EngagementPrediction>
  >({});
  ...
  const notificationTemplates = [
    {
      ...
      name: "Fado Night Event",
      name_pt: "Evento de Fado",
    },
    {
      id: "business_networking_portuguese",
      name: "Business Networking",
      name_pt: "Networking Empresarial",
    },
    {
      id: "festival_santos_populares",
      name: "Santos Populares",
      name_pt: "Santos Populares",
    },
  ];
  ...
  const loadAIAnalytics = useCallback(async () => {
    ...
    try {
      ...
      const analyticsData =
        await notificationService.getAINotificationAnalytics();
      if (isMounted()) {
  ...
  const loadEngagementPredictions = useCallback(async () => {
    ...
    try {
      ...
      const predictionPromises = notificationTemplates.map(async (template) => {
        try {
          const prediction =
            await notificationService.predictNotificationEngagement(
              "demo-user-id",
              template.id
            );
          ...
          return {
            ...
            prediction: {
              ...
              optimal_send_time: ["19:00", "20:00", "21:00"][
                Math.floor(Math.random() * 3)
              ],
              recommendations: [
                language === "pt"
                  ? "IA: Alta relevância cultural detectada"
                  : "AI: High cultural relevance detected",
                language === "pt"
                  ? "ML: Horário noturno otimiza engagement"
                  : "ML: Evening timing optimizes engagement",
                language === "pt"
                  ? "NLP: Conteúdo português recomendado"
                  : "NLP: Lusophone content recommended",
              ],
  ...
  const getEngagementLabel = useCallback(
    (score: number) => {
      if (score >= 80)
        return language === "pt" ? "IA: Excelente" : "AI: Excellent";
  ...
  const runCulturalABTest = useCallback(async () => {
    try {
      ...
      const variants = [
        {
          id: "formal_pt",
          modifications: { tone: "formal", cultural_emphasis: "high" },
        },
        {
          id: "casual_en",
          modifications: { tone: "casual", cultural_emphasis: "medium" },
        },
      ];
  ...
  const [performanceMetrics, setPerformanceMetrics] =
    useState<NotificationPerformanceMetrics | null>(null);
    ...
    return (
      <div className="flex items-center justify-center p-8">
        ...
        <span className="ml-3 text-gray-600">
          {language === "pt"
            ? "IA processando dados da comunidade de falantes de português..."
            : "AI processing Portuguese-speaking community data..."}
        </span>
        <div className="ml-4 text-xs text-gray-500">
          {language === "pt"
            ? `Tempo: ${Math.round(metrics.loadTime)}ms`
            : `Time: ${Math.round(metrics.loadTime)}ms`}
        </div>
      </div>
    );
  }
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-2xl font-bold">
              {language === "pt"
                ? "IA Premium - Notificações Culturais"
                : "Premium AI - Cultural Notifications"}
            </h1>
            ...
            <div className="flex items-center gap-4 mt-2 text-sm text-white/80">
              ...
              <span>
                📊 {language === "pt" ? "ML Avançado" : "Advanced ML"}
              </span>
              <span>
                🇵🇹 {language === "pt" ? "Otimizado PT" : "PT Optimized"}
              </span>
            </div>
          </div>
        </div>
            ...
            >
              {portugueseRegions.map((region) => (
                <option key={region.code} value={region.code}>
                  {region.flag}{" "}
                  {language === "pt" ? region.name_pt : region.name}
                </option>
              ))}
            </select>
          </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div
                        className={`px-2 py-1 rounded-full text-xs font-medium ${getEngagementColor(prediction.likelihood_score)}`}
                      >
                        {getEngagementLabel(prediction.likelihood_score)} (
                        {Math.round(prediction.likelihood_score)}%)
                      </div>
                    </div>
        ...
        {expandedSections.cultural && (
          <div className="p-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {Object.entries(analytics?.cultural_patterns || {}).map(
                ([region, pattern]: [string, any]) => {
                  const regionInfo = portugueseRegions.find(
                    (r) => r.code === region
                  );
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div
                            ...
                            style={{
                              width: `${pattern.engagement_rate * 100}%`,
                            }}
                          />
                        </div>
