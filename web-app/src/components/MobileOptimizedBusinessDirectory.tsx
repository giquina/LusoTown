...
import { FEATURED_PORTUGUESE_BUSINESSES } from "@/config/business-directory-carousels";
import {
  useMobileScrollAnimation,
  useCardGridAnimation,
  usePortugueseScrollAnimation,
} from "@/hooks/useMobileScrollAnimation";
import {
  MagnifyingGlassIcon,
  MapPinIcon,
  FunnelIcon,
...
export default function MobileOptimizedBusinessDirectory({
  ...
  // Note: Use FEATURED_PORTUGUESE_BUSINESSES as the source list
  const allBusinesses = FEATURED_PORTUGUESE_BUSINESSES;
  ...
  const filteredBusinesses = allBusinesses
    .filter((business) => {
      const matchesSearch =
        ...
        business.description.pt
          .toLowerCase()
          .includes(searchTerm.toLowerCase());

      const matchesLocation =
        selectedLocation === "all" ||
        business.location.city === selectedLocation;
      const matchesCategory =
        selectedCategory === "all" || business.category === selectedCategory;
      const matchesFeatured = !featured || business.isFeatured;

      return (
        matchesSearch && matchesLocation && matchesCategory && matchesFeatured
      );
    })
    .slice(0, maxItems);
  ...
  const uniqueLocations = Array.from(
    new Set(allBusinesses.map((b) => b.location.city))
  );
    ...
    <div className={`mobile-safe ${className}`}>
      ...
      >
        ...
        <p className="text-portuguese-mobile text-gray-600 max-w-3xl mx-auto">
          {t(
            "business.directory.subtitle",
            "Discover authentic Portuguese businesses, restaurants, and services across the United Kingdom"
          )}
        </p>
      ...
      {showSearch && (
        <div ref={searchRef} className="section-spacing scroll-fade-in">
          {/* Search Bar */}
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder={t(
                  ...
                  "Search businesses, restaurants, services..."
                )}
                value={searchTerm}
              ...
              <div className="border-t border-gray-200 touch-spacing-large animate-fade-in">
                <div className="grid grid-cols-1 sm:grid-cols-2 touch-grid-large">
                  ...
                  <div>
                    ...
                    >
                      <option value="all">
                        {t("business.filters.all_locations", "All Locations")}
                      </option>
                  ...
                  <div>
                    ...
                    >
                      <option value="all">
                        {t("business.filters.all_categories", "All Categories")}
                      </option>
      ...
      <div className="text-center touch-spacing text-portuguese-mobile text-gray-600">
        {t(
          ...
          `Showing ${filteredBusinesses.length} Portuguese businesses`
        )}
      </div>

      {/* Business Cards Grid/List */}
      <div ref={businessesRef} className="section-spacing lusophone-reveal">
      ...
      {filteredBusinesses.length === maxItems &&
        allBusinesses.length > maxItems && (
