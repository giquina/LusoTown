...
import { transportCache, performanceMonitor, debounceManager } from '@/lib/transportCache'
import { pricingEngine } from '@/lib/transportPricing'
import OptimizedTransportServiceCard from './OptimizedTransportServiceCard'
import TransportTestimonials from './TransportTestimonials'
...
const OptimizedTransportPage: React.FC<OptimizedTransportPageProps> = ({
  ...
  const serviceTiers = useMemo(() => [
    {
      ...
      price: 45,
      description: 'Professional private transport service with premium vehicle',
      descriptionPortuguese: 'Serviço profissional de transporte privado com veículo premium',
      features: [
        'Professional private hire licensed driver with CPO security training',
        'Premium vehicle fleet (Mercedes S-Class, BMW 7 Series)',
  ...
  const handleBookService = useCallback((serviceId: string) => {
    ...
    if (selectedDate) {
      const cacheKey = transportCache.pricing.generatePricingKey(
        serviceId,
        'tier',
        duration,
        selectedDate,
      ...
      if (!transportCache.pricing.has(cacheKey)) {
...
export default React.memo(OptimizedTransportPage)