...
export default function MobileNavigation({ user, onClose, isOpen }: MobileNavigationProps) {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <>
          {/* Mobile menu backdrop - Enhanced z-index for proper layering */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          {/* Mobile menu content - Enhanced with proper z-index and styling */}
          <motion.div
            ...
            className="xl:hidden fixed top-[80px] left-2 right-2 z-[9999] bg-white/98 backdrop-blur-xl border border-primary-200/50 shadow-2xl rounded-2xl max-h-[85vh] overflow-y-auto"
            style={{
                  ...
                  <div className="space-y-2">
                    {section.items.map((item) => (
                      <motion.a
                        key={item.name}
                        href={item.href}
                        className="luxury-touch-target flex items-center gap-3 text-gray-700 hover:text-primary-600 hover:bg-primary-50 p-4 rounded-xl text-base font-medium transition-all duration-300 border border-transparent hover:border-primary-200 min-h-[56px] group"
                        onClick={handleLinkClick}
                        initial={{ opacity: 0, x: -20 }}
                        animate={{ opacity: 1, x: 0 }}
                        transition={{ delay: sectionIndex * 0.1 + 0.05, type: "spring", stiffness: 400, damping: 25 }}
                        whileHover={{ scale: 1.02, x: 4 }}
                        whileTap={{ scale: 0.98 }}
                      >
                        <item.icon className="w-5 h-5 flex-shrink-0 text-primary-500 transition-transform duration-300 group-hover:scale-110" />
                ...
                {user ? (
                  <div className="space-y-3">
                    ...
                    <motion.a
                      ...
                      className="flex items-center gap-3 text-gray-700 hover:text-primary-600 hover:bg-primary-50 p-4 rounded-xl text-base font-medium transition-all duration-300 border border-transparent hover:border-primary-200 min-h-[56px] luxury-touch-target group"
                      onClick={handleLinkClick}
                      whileHover={{ scale: 1.02, x: 4 }}
                      whileTap={{ scale: 0.98 }}
                      transition={{ type: "spring", stiffness: 400, damping: 25 }}
                    >
                      <UserIcon className="w-5 h-5 text-primary-500 transition-transform duration-300 group-hover:scale-110" />
                    ...
                    <motion.a
                      ...
                      transition={{ type: "spring", stiffness: 400, damping: 25 }}
                    >
                      <HeartIcon className="w-5 h-5 text-primary-500 transition-transform duration-300 group-hover:scale-110" />
                      <span>My Favourites</span>
                    </motion.a>

                    <motion.button
                      className="flex items-center gap-3 text-action-600 hover:text-action-700 hover:bg-action-50 p-4 rounded-xl text-base font-medium transition-all duration-300 w-full text-left border border-transparent hover:border-action-200 min-h-[56px] luxury-touch-target group"
                      onClick={() => {
                        // Handle logout
                        handleLinkClick();
                      }}
                      ...
                      transition={{ type: "spring", stiffness: 400, damping: 25 }}
                    >
                      <LogOut className="w-5 h-5 text-action-500 transition-transform duration-300 group-hover:scale-110" />
                      <span>Sign Out</span>
                    </motion.button>
                  </div>
                ) : (
                  <div className="space-y-3">
                    ...
                    <motion.a
                      href={ROUTES.signup}
                      className="bg-gradient-to-r from-secondary-600 via-action-600 to-accent-600 text-white font-bold py-4 px-6 rounded-xl shadow-lg hover:from-secondary-700 hover:via-action-700 hover:to-accent-700 hover:shadow-xl transition-all duration-300 w-full text-center min-h-[56px] flex items-center justify-center luxury-touch-target relative overflow-hidden"
                      onClick={handleLinkClick}
                      whileHover={{ scale: 1.02 }}
                      whileTap={{ scale: 0.98 }}
                      transition={{ type: "spring", stiffness: 400, damping: 25 }}
