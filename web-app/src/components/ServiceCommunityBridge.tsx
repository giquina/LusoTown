...
export default function ServiceCommunityBridge({
  ...
  const getContextualIntegrations = () => {
    switch (triggerContext) {
        ...
        return {
          ...
          integrations: [
            ...
            },
            {
              ...
              title: isPortuguese
                ? "Transporte + Networking"
                : "Transport + Networking",
              description: isPortuguese
                ? "Compartilhe viagens com outros portugueses da sua área"
                : "Share rides with other Portuguese speakers in your area",
              value: isPortuguese
                ? "Conexões instantâneas + Custos compartilhados"
                : "Instant connections + Shared costs",
              icon: UserGroupIcon,
              color: "accent",
            },
          ],
      ...
      case "event_page":
        return {
          title: isPortuguese
            ? "Chegue ao Evento em Grande Estilo"
            : "Arrive at the Event in Style",
          subtitle: isPortuguese
            ? "Transforme sua ida ao evento numa experiência premium"
            : "Turn your trip to the event into a premium experience",
          integrations: [
            {
              id: "event-transport-vip",
              title: isPortuguese
                ? "Transporte VIP para o Evento"
            ...
            },
            {
              ...
              description: isPortuguese
                ? "Compartilhe o transporte com outras pessoas que vão ao mesmo evento"
                : "Share transport with others going to the same event",
              value: isPortuguese
                ? "Economize 30% + Conhece pessoas"
                : "Save 30% + Meet people",
              icon: Users,
              color: "secondary",
            },
      ...
      case "community_page":
        return {
          title: isPortuguese
            ? "Leve sua Experiência Comunitária ao Próximo Nível"
            : "Take Your Community Experience to the Next Level",
          subtitle: isPortuguese
            ? "Adicione serviços premium às suas atividades comunitárias"
            : "Add premium services to your community activities",
          integrations: [
            {
              id: "community-premium-services",
              title: isPortuguese
                ? "Serviços Premium para Membros"
                : "Premium Services for Members",
              description: isPortuguese
                ? "Acesso a transporte executivo e segurança para eventos da comunidade"
                : "Access to executive transport and security for community events",
              value: isPortuguese
                ? "Desconto exclusivo de membro + Serviços de qualidade"
                : "Exclusive member discount + Quality services",
              icon: Crown,
              color: "premium",
            },
          ],
      ...
      default:
        return {
          title: isPortuguese
            ? "Uma Plataforma, Todas as Experiências"
            : "One Platform, All Experiences",
          subtitle: isPortuguese
            ? "Descubra como transportes e comunidade se conectam na LusoTown"
            : "Discover how transport and community connect at LusoTown",
  ...
  const handleGroupBooking = async () => {
    if (selectedEvent && selectedConnections.length > 0) {
      const success = await createGroupTransportBooking(
        selectedEvent.id,
        selectedConnections
      );
      if (success) {
          ...
          >
            ...
            <div className="flex-1">
              ...
              <div className="text-sm text-gray-500">
                ...
                {connection.sharedEventsCount}{" "}
                {isPortuguese ? "eventos juntos" : "events together"}
              </div>
            </div>
          </div>
        ))}
