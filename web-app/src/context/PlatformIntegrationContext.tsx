...
interface PlatformIntegrationContextType {
  ...
  addNotification: (
    notification: Omit<CrossPlatformNotification, "id" | "createdAt">
  ) => void;
...
const PlatformIntegrationContext = createContext<
  PlatformIntegrationContextType | undefined
>(undefined);

export function PlatformIntegrationProvider({
  children,
}: {
  children: ReactNode;
}) {
  ...
  const [ecosystemAnalytics, setEcosystemAnalytics] =
    useState<EcosystemAnalytics>({
  ...
  const [activeRecommendations, setActiveRecommendations] = useState<
    ServiceRecommendation[]
  >([]);
  const [crossPlatformNotifications, setCrossPlatformNotifications] = useState<
  ...
  const newRecommendations = generateAutomaticRecommendations();
      setActiveRecommendations((prev) => {
        ...
        const unique = combined.filter(
          (rec, index, self) =>
            index ===
            self.findIndex((r) => r.type === rec.type && r.title === rec.title)
        );
        return unique
          .slice(0, 10)
  ...
  const loadPlatformData = () => {
    try {
      ...
      const savedNotifications = localStorage.getItem(
        "lusotown-platform-notifications"
      );
  ...
  const savePlatformData = () => {
    try {
      localStorage.setItem(
        "lusotown-user-journey",
        JSON.stringify(userJourney)
      );
      localStorage.setItem(
        "lusotown-ecosystem-analytics",
        JSON.stringify(ecosystemAnalytics)
      );
      localStorage.setItem(
        "lusotown-recommendations",
        JSON.stringify(activeRecommendations)
      );
      localStorage.setItem(
        "lusotown-platform-notifications",
        JSON.stringify(crossPlatformNotifications)
      );
    } catch (error) {
  ...
  const updateEcosystemAnalytics = useCallback(() => {
    ...
    setEcosystemAnalytics((prev) => ({
      ...
      communityParticipationLevel: Math.min(
        10,
        safeNetworkStats.eventsAttended * 0.5 + safeConnections.length * 0.3
      ),
  ...
  const initializeUserJourney = useCallback(
    (startPoint: string) => {
      const newJourney: UserJourney = {
        id: `journey-${Date.now()}`,
        userId: "current-user",
        journeyType: startPoint.includes("transport")
          ? "service_to_community"
          : "community_to_service",
        startPoint,
    ...
    (
      ...
      if (trigger === "high_networking_activity" && connections.length >= 3) {
        recommendations.push({
          ...
          type: "transport",
          title: isPortuguese
            ? "Transporte VIP para Eventos"
            : "VIP Transport for Events",
          description: isPortuguese
      ...
      if (
        trigger === "membership_opportunity" &&
        !hasActiveSubscription &&
        ecosystemAnalytics.userEngagementScore > 5
      ) {
        recommendations.push({
  ...
  const generateAutomaticRecommendations =
    useCallback((): ServiceRecommendation[] => {
      const triggers = [];
      ...
      if (
        !hasActiveSubscription &&
        ecosystemAnalytics.userEngagementScore > 4
      ) {
  ...
  const suggestCommunityEngagement = useCallback(
    (serviceType: string): ServiceRecommendation[] => {
      const isPortuguese = language === "pt";
      ...
      if (serviceType === "transport") {
        suggestions.push({
          ...
          title: isPortuguese
            ? "Grupo de Exploradores de Londres"
  ...
  const suggestRelevantServices = useCallback(
    (
      communityActivity: string,
      connections: any[]
    ): ServiceRecommendation[] => {
  ...
  const handleNetworkingEventAttendance = useCallback(
    (eventId: string, connections: any[]) => {
      ...
      updateJourneyProgress("networking_event_attended", {
        eventId,
  ...
  const generateBusinessNetworkingOpportunities = useCallback(
    (networkData: Record<string, any>): ServiceRecommendation[] => {
      const isPortuguese = language === "pt";
  ...
  const getPersonalizedRecommendations =
    useCallback((): (ServiceRecommendation & { benefits?: string[] })[] => {
    ...
    () => [
      {
        ...
        description:
          language === "pt"
            ? "Combine transporte premium com eventos culturais"
            : "Combine premium transport with cultural events",
        serviceIntegration: {
          transportIncluded: true,
          eventAccess: true,
          networkingFeatures: false,
        },
      ...
      },
      {
        id: "bridge-2",
        title:
          language === "pt" ? "Networking + Serviços" : "Networking + Services",
        category: "community",
        description:
          language === "pt"
            ? "Conecte-se e reserve serviços em conjunto"
            : "Connect and book services together",
        serviceIntegration: {
    ...
    return [
      {
        ...
        culturalExperience:
          language === "pt"
            ...
            : "Live Fado with optional transport",
        groupSavings:
          language === "pt" ? "Poupe 20% em grupos" : "Save 20% in groups",
  ...
  const createGroupTransportBooking = useCallback(
  ...
  const addNotification = useCallback(
    (notification: Omit<CrossPlatformNotification, "id" | "createdAt">) => {
      ...
      setCrossPlatformNotifications((prev) =>
  ...
  const trackRevenueOpportunity = useCallback(
    (amount: number, source: string, metadata: Record<string, any> = {}) => {
      setEcosystemAnalytics((prev) => ({
        ...prev,
  ...
  const generateUserInsights = useCallback(() => {
    ...
    if (userEngagementScore > 7) {
      insights.strengths.push(
        isPortuguese
          ? "Alto envolvimento na plataforma"
    ...
    if (!hasActiveSubscription && userEngagementScore > 5) {
      insights.opportunities.push(
        isPortuguese
          ? "Candidato ideal para membership premium"
          : "Great candidate for premium membership"
      );
    }
    ...
    insights.nextActions.push(
      isPortuguese
        ? "Participar no próximo evento da comunidade"
    ...
    if (!hasActiveSubscription) {
      insights.nextActions.push(
        isPortuguese
          ? "Considerar membership para descontos"
          : "Consider membership for discounts"
      );
    }
...
export function usePlatformIntegration() {
  ...
  if (context === undefined) {
    throw new Error(
      "usePlatformIntegration must be used within a PlatformIntegrationProvider"
    );
  }
  return context;
}
