...
interface SaudadeAssessment {
  id: string;
  type: 'intensity' | 'frequency' | 'triggers' | 'coping' | 'connection' | 'language_emotion';
  category: 'emotional_depth' | 'cultural_triggers' | 'heritage_connection' | 'support_needs' | 'homeland_bond';
  weight: number;
  titleEn: string;
  titlePt: string;
...
const saudadeAssessmentQuestions: SaudadeAssessment[] = [
  ...
  
  {
    ...
    options: [
      ...
      { id: 'rarely', labelEn: 'Rarely, only on very special occasions', labelPt: 'Raramente, só em ocasiões muito especiais', value: 2, emoji: '✨' },
    ],
  },
  ...
  // Saudade Triggers - Deep Cultural Elements
  {
    ...
    weight: 4.5,
    titleEn: 'Which elements trigger your deepest saudade? (Select all that apply)',
    titlePt: 'Que elementos despertam a sua saudade mais profunda? (Selecione todos os que se aplicam)',
    options: [
  ...
  // Regional Portuguese Identity Depth
  {
    ...
    options: [
      ...
      { id: 'porto_norte', labelEn: 'Porto/Norte - Proud Northerner, different from South', labelPt: 'Porto/Norte - Nortista orgulhoso(a), diferente do Sul', value: 10, emoji: '🍷' },
      { id: 'lisboa_area', labelEn: 'Lisboa area - Capital energy and culture', labelPt: 'Zona de Lisboa - energia e cultura da capital', value: 10, emoji: '🏛️' },
      { id: 'centro_coimbra', labelEn: 'Centro/Coimbra - Academic and historical heritage', labelPt: 'Centro/Coimbra - herança académica e histórica', value: 10, emoji: '🎓' },
      { id: 'alentejo_deep', labelEn: 'Alentejo - Countryside soul and traditions', labelPt: 'Alentejo - alma do campo e tradições', value: 10, emoji: '🌾' },
  ...
  // Cultural Support Needs Assessment
  {
    ...
    options: [
      { id: 'understanding_saudade', labelEn: 'Someone who understands my saudade without explanation', labelPt: 'Alguém que entenda a minha saudade sem explicação', value: 10, emoji: '🤝' },
...
interface SaudadeCompatibilityResult {
  ...
  connectionType: 'saudade_soulmate' | 'cultural_healer' | 'heritage_guardian' | 'integration_partner' | 'gentle_companion';
}
...
export default function SaudadeMatchingSystem({ 
  ...
  const currentQuestion = saudadeAssessmentQuestions[currentQuestionIndex];
  const progress = ((currentQuestionIndex + 1) / saudadeAssessmentQuestions.length) * 100;
  const isLastQuestion = currentQuestionIndex === saudadeAssessmentQuestions.length - 1;

  // Initialize slider value for each question
  useEffect(() => {
    if (currentQuestion?.sliderConfig) {
      const mid = Math.ceil((currentQuestion.sliderConfig.min + currentQuestion.sliderConfig.max) / 2);
      setSliderValue(mid);
    }
    setSelectedOptions([]);
  ...
  const calculateSaudadeProfile = (assessmentAnswers: Record<string, any>): CulturalDepthProfile => {
    // Calculate saudade intensity
    const saudadeIntensity = assessmentAnswers['saudade_intensity_core']?.value || 5;
    const homelandConnection = assessmentAnswers['homeland_connection_strength']?.value || 5;
    const languageEmotional = assessmentAnswers['language_emotional_attachment']?.value || 5;
    const heritageBalance = assessmentAnswers['heritage_integration_balance']?.value || 5;

    // Determine frequency pattern
    const frequencyAnswers = assessmentAnswers['saudade_frequency_patterns']?.value || [];
    let frequency: SaudadeProfile['frequency'] = 'monthly';
    if (frequencyAnswers.includes('daily_constant')) frequency = 'constant';
    else if (frequencyAnswers.includes('evening_nights') || frequencyAnswers.includes('music_triggers')) frequency = 'weekly';
    ...
    const copingMechanisms = assessmentAnswers['saudade_coping_mechanisms']?.value || [];
    const supportNeeds = assessmentAnswers['cultural_support_needs']?.value || [];

    // Determine cultural support level
    let culturalSupport: SaudadeProfile['culturalSupport'] = 'moderate';
    if (supportNeeds.includes('understanding_saudade') || supportNeeds.includes('cultural_healing')) {
    ...
    const regionalAnswers = assessmentAnswers['regional_identity_depth']?.value || [];
    ...
    if (saudadeIntensity >= 8 && culturalSupport === 'high') {
      emotionalCompatibilityType = language === 'pt' ? 'Alma Saudosa' : 'Saudade Soul';
    } else if (heritageBalance >= 8 && homelandConnection >= 8) {
      emotionalCompatibilityType = language === 'pt' ? 'Guardião Cultural' : 'Cultural Guardian';
    } else if (culturalSupport === 'high' && copingMechanisms.includes('portuguese_community')) {
      emotionalCompatibilityType = language === 'pt' ? 'Construtor de Pontes' : 'Bridge Builder';
    } else if (saudadeIntensity >= 6 && frequency === 'weekly') {
      emotionalCompatibilityType = language === 'pt' ? 'Coração Nostálgico' : 'Nostalgic Heart';
    } else {
      emotionalCompatibilityType = language === 'pt' ? 'Equilibrista Cultural' : 'Cultural Balancer';
    }

    // Generate cultural healing activities
    const culturalHealingActivities = generateCulturalHealingActivities(triggers, copingMechanisms, language);
    ...
    const overallCulturalDepth = (saudadeIntensity + homelandConnection + languageEmotional + heritageBalance) / 4;
    ...
    const culturalDepthProfile: CulturalDepthProfile = {
      ...
      communityInvolvement: culturalSupport === 'high' ? 8 : culturalSupport === 'moderate' ? 6 : 4,
      ...
      musicArtConnection: { fado: triggers.includes('fado_music') ? 9 : 5, folk: 6 },
      foodCookingInvolvement: copingMechanisms.includes('cook_portuguese') ? 9 : 5,
      socialCustomsAdherence: heritageBalance,
      communityLeadership: culturalSupport === 'high' ? 7 : 4,
      overallCulturalDepth,
      compatibilityRecommendations: generateCompatibilityRecommendations(saudadeProfile, language),
    };
  ...
  const generateCulturalHealingActivities = (
    ...
    if (triggers.includes('fado_music') || copingMechanisms.includes('listen_fado')) {
      activities.push(
        lang === 'pt' 
  ...
  const renderQuestion = () => {
    switch (currentQuestion.type) {
      ...
      case 'triggers':
      case 'coping':
        const isMultiSelect = currentQuestion.type === 'triggers' || currentQuestion.type === 'coping';
        return (
      ...
      >
        ...
        >
          ...
          <div className="space-y-6">
            ...
            >
              {language === 'pt' ? 'Ver Matches Emocionalmente Compatíveis' : 'View Emotionally Compatible Matches'}
            </button>
    ...
    <>
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 bg-gradient-to-r from-red-500 to-primary-500 rounded-full flex items-center justify-center">
      ...
      <div className="flex justify-between items-center">
        ...
        >
          ...
          {isLastQuestion
            ? (language === 'pt' ? 'Completar Avaliação' : 'Complete Assessment')
